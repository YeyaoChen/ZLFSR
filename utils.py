...
def calculate_total_scores(pred_y, gt_y, csv_name, lfi_no, out_ang2):
    view_list = []
    view_psnr_y = []
    view_ssim_y = []
    ...
    dataframe_lfi = pd.DataFrame({'Reconstructed_LFI{}'.format(lfi_no): view_list, 'PSNR Y': view_psnr_y, 'SSIM Y': view_ssim_y})
    dataframe_lfi.to_csv(csv_name, index=False, sep=',', mode='a')
    return np.mean(view_psnr_y), np.mean(view_ssim_y)


def save_LFimg(pred_y, gt_ycbcr, save_path, lfi_no, out_ang, isWarp=True):
    # pred_y: [b,an2,h,w], gt_ycbcr: [b,an2,h,w,3]
    ...
    for t in range(out_ang * out_ang):
        ycbcr_im = gt_ycbcr[0, t]           # [1,1,h,w,3]
        ycbcr_im[:, :, 0] = pred_y[0, t]    # Y channel [h,w,3]
