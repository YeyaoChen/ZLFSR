...
%% path
main_folder = 'I:/Multi_view_LF/';
folder1 = [main_folder,'leftCam/'];
% folder2 = [main_folder,'centerCam/'];
folder2 = [main_folder,'rightCam/'];
save_path = 'dataset_LR/';
...
for x = 1:1
    ind = index(x);
    savename = [save_path,'data',num2str(x),'.h5'];
    ...
    for p = 1:7
        for q = 1:7
            ...
            sai2 = imread([folder2,num2str(ind),'/',num2str(p),'_',num2str(q),'.png']);
            sai2 = rgb2ycbcr(sai2);
            img2(:,:,:,p,q) = sai2;
        end
    end
