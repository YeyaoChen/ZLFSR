...
parser.add_argument("--dataset_path", type=str, default="matlab/RealData/data", help="Training data path")
...
for LF_num in range(1,2):
    # model save path
    model_dir = 'trained_model/scene' + str(LF_num)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    # Training save image
    if cfg.Is_buffer:
        train_visual_dir = 'training_buffer/scene' + str(LF_num)
    ...
    def train(epoch, start_time):
        ...
        for tp in range(15):
            for tq, batch in enumerate(train_loader):
                ...
                ##################### Forward inference #####################
                warp_center_stack, warp_single_saiA, warp_single_saiB, pred_flowA, pred_flowB, \
                pred_depthA, pred_depthB, \
