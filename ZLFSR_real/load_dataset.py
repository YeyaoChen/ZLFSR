...
class TestSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_cb2 = lfi_ycbcr2[ind_source, :, :, 1]     # [1,h,w]
        input_cr2 = lfi_ycbcr2[ind_source, :, :, 2]     # [1,h,w]

        input1 = torch.from_numpy(input1.astype(np.float32)/255.0)
        input2 = torch.from_numpy(input2.astype(np.float32)/255.0)
        input_cb2 = torch.from_numpy(input_cb2.astype(np.float32)/255.0)
        input_cr2 = torch.from_numpy(input_cr2.astype(np.float32)/255.0)

        return input1, input2, input_cb2, input_cr2
